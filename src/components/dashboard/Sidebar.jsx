    ...
    <aside className="left-sidebar" ref={sidebarRef}>
      ...
      <ul className="subjects-list" style={{ marginBottom: '8px' }}>
        {mockData.navLinks.map((link, index) => (
          <li key={index}>
            <a 
              href={link.name === 'Arena' ? '/arena' : link.name === 'Leaderboard' ? '/leaderboard' : link.name === 'Calendar' ? '/calendar' : link.name === 'My Timeline' ? '/timeline' : link.name === 'Question of the Day' ? '/qotd' : link.name === 'Scorecard' ? '/scorecard' : '#'} 
              className={`sidebar-item ${path === (link.name === 'My Timeline' ? '/timeline' : link.name === 'Question of the Day' ? '/qotd' : '/' + link.name.toLowerCase()) ? 'active' : ''}`} 
              onClick={(e) => {
